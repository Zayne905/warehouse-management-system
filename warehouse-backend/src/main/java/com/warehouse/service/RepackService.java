...
public class RepackService {
    ...
    public RepackOrderVO addDetailLine(Long orderId, String sourceKanbanNo, Integer transferQty) {
        ...
        if ("CONSOLIDATE".equals(type)) {
            ...
            transferQty = source.getQuantity().intValue(); // 自动取全部剩余量
        }
        ...
        if ("REMAINDER".equals(type)) {
            ...
            if (transferQty >= source.getQuantity().intValue())
        ...
        if ("BREAKDOWN".equals(type) && !existDetails.isEmpty()) {
            ...
            if (sum + transferQty > source.getQuantity().intValue())
    ...
    public RepackOrderVO breakdownAdd(Long orderId, Integer targetBoxCapacity) {
        ...
        int totalQty = source.getQuantity().intValue();  // 源箱总数量（拆包必须全拆）
    ...
    private List<Kanban> execBreakdown(List<RepackOrderDetail> details, RepackOrder order) {
        ...
        if (total > source.getQuantity().intValue()) throw new RuntimeException("总拆出量(" + total + ")超过源数量(" + source.getQuantity() + ")");
        ...
        for (RepackOrderDetail d : details) {
            ...
            updateDetailResult(d, no, source.getQuantity().intValue() - total);
        }
        source.setQuantity(source.getQuantity().subtract(java.math.BigDecimal.valueOf(total)));
        source.setStatus(source.getQuantity().compareTo(java.math.BigDecimal.ZERO) <= 0 ? Kanban.STATUS_CLEARED : Kanban.STATUS_PARTIAL_REPACK);
    ...
    private List<Kanban> execConsolidate(List<RepackOrderDetail> details, RepackOrder order) {
        ...
        for (int i = 0; i < sources.size(); i++) {
            ...
            int rem = s.getQuantity().intValue() - d.getTransferQty();
            s.setQuantity(java.math.BigDecimal.valueOf(rem));
    ...
    private List<Kanban> execRemainder(List<RepackOrderDetail> details, RepackOrder order) {
        ...
        int remaining = source.getQuantity().intValue() - d.getTransferQty();
        ...
        source.setQuantity(java.math.BigDecimal.valueOf(remaining));
    ...
    private void autoInbound(RepackOrder order, List<Kanban> newKanbans) {
        ...
        for (Kanban k : newKanbans) {
            ...
            detail.setUnit("件");
            detail.setPlannedQty(k.getQuantity());
            detail.setActualQty(k.getQuantity());
            detail.setWarehouseAreaId(k.getWarehouseAreaId());
            detail.setBoxCount(java.math.BigDecimal.ONE);
            detail.setLineNo(1);
    ...
    private Kanban newTargetKanban(String kanbanNo, Kanban source, int quantity) {
        ...
        t.setQuantity(java.math.BigDecimal.valueOf(quantity)); t.setOriginalQty(java.math.BigDecimal.valueOf(quantity));
        t.setBoxSeq(0);
    ...
    private Kanban reloadAndRecheck(RepackOrderDetail detail) {
        ...
        if (detail.getTransferQty() > s.getQuantity().intValue())
