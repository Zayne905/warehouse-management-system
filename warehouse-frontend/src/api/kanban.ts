...
export interface KanbanLifecycle {
  kanban: Kanban & {
  ...
  events: Array<{ time?: string; type: string; title: string; orderNo?: string }>
}

export function getKanbanLifecycleApi(kanbanNo: string): Promise<{
  code: number
  message: string
  data: KanbanLifecycle
}> {
  return request.post('/kanban/lifecycle', { kanbanNo })
}

export interface KanbanListQuery {
  keyword?: string
  status?: number | ''
  warehouseAreaName?: string
  supplierName?: string
...
export function listKanbansApi(query: KanbanListQuery): Promise<{
  code: number
  message: string
  data: Kanban[]
}> {
  return request.post('/kanban/list', query)
}
...
/** 按勾选的看板批量封存 */
export function batchBlockApi(kanbanNos: string[]): Promise<{
  code: number
  message: string
  data: number
}> {
  return request.post('/kanban/batch-block', { kanbanNos })
}
